<%@ page import="data.Class" %><%--
  ...
  File name(文件名): insert_a_class_result
  Author(作者）: mao
...
<%
    ...
    {
        //把错误信息写入session里
        session.setAttribute("message", "该班级已经存在！");
        //转发至错误页面
        request.getRequestDispatcher("Error.jsp").forward(request, response);
        return;
    }
    ...
    //插入班级
    boolean result = table.Class.insert(class_no, class_name, class_college, class_major, class_enrollment_year_str);
    if (!result)
    {
        //把错误信息写入session里
        session.setAttribute("message", "班级插入失败！");
        //转发至错误页面
        request.getRequestDispatcher("Error.jsp").forward(request, response);
        return;
    }
...
<%
    //重定向
    response.sendRedirect("see_class.jsp");
%>
</body>
</html>
