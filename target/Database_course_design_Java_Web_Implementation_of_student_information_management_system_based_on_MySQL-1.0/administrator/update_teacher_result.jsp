<%--
  ...
  File name(文件名): update_teacher_result
  Author(作者）: mao
...
<%
    ...
    //更新教师信息
    boolean result = table.Teacher.update(teacher_no, teacher_name, teacher_sex, teacher_telephone_number, teacher_email, teacher_address, teacher_idcard);
    if (!result)
    {
        //把错误信息写入session里
        session.setAttribute("message", "教师信息更新失败！");
        //转发至错误页面
        request.getRequestDispatcher("Error.jsp").forward(request, response);
        return;
    }
