...
* （2）用Java实现学生信息管理系统，可以是GUI图形界面实现；可以是控制台输出；还可以java web实现；可以（基于Maven)用SSM框架实现；可以用springboot实现，前台使用Java
  语言，后台使用MYSQL数据库，或其他关系数据库来开发本系统。可以使用eclipse\myeclipse\idea开发环境,可以实现Java GUI图形界面、控制台实现、java web (MVC)
  实现、SSM框架实现、springBoot实现。
* （3）总体设计方案的分析、比较和论证。
